...
def place_session(session_id: str, body: PlaceBody):
    ...
    def fn(cfg):
        ...
        if body.group_id:
            ...
            # ADOPT THE STRAGGLERS FIRST. A group's rows render as "ordered
            # members, THEN anything merely assigned but never ordered" — so a
            # drop at the bottom appended to the order list yet still drew ABOVE
            # those unordered rows, snapping the session back up. ("I can drag to
            # the top of a group but not the bottom.") Pull every current member
            # into the list, in the exact order they're displayed, so the list is
            # authoritative and "append" really means last.
            for m, gid in assign.items():
                if gid == body.group_id and m != local and m not in members:
                    members.append(m)
            idx = len(members)  # default: append (bottom)
